...
function loadSettingsForm() {
  ...
  if (configToggleInput) {
    var savedConf = localStorage.getItem('invoiceflow_firebase_config');
...
function saveBusinessProfile() {
  ...
  if (configInput) {
    var oldConf = localStorage.getItem('invoiceflow_firebase_config') || '';
    var newConf = configInput.value.trim();
    if (newConf !== oldConf) {
      localStorage.setItem('invoiceflow_firebase_config', newConf);
...
function getFirebaseConfig() {
  ...
  try {
    var conf = localStorage.getItem('invoiceflow_firebase_config');
...
function initializeSandbox() {
  ...
  var savedUser = loadData('invoiceflow_current_user');
  if (savedUser) {
...
function handleLogin() {
  ...
  var users = loadData('invoiceflow_simulated_users') || {};
  ...
  if (user && user.password === password) {
    ...
    saveData('invoiceflow_current_user', currentUser);
    handleLoggedInState();
...
function handleRegister() {
  ...
  saveData('invoiceflow_simulated_users', users);
...
function triggerSignOut() {
  ...
  localStorage.removeItem('invoiceflow_current_user');
  handleLoggedOutState();
...
window.addEventListener('appinstalled', function(evt) {
  console.log('[PWA] InvoiceFlow installed successfully!');
  deferredPrompt = null;
  updatePWAInstallUI();
  showToast('InvoiceFlow installed successfully!', 'success');
});
...
function dismissSwipeHint() {
  localStorage.setItem('invoiceflow_dismissed_swipe_hint', 'true');
  var banner = document.getElementById('swipe-hint-banner');
  if (banner) {
...
function updatePWAInstallUI() {
  ...
  if (isStandalone) {
    ...
    if (desc) desc.textContent = 'InvoiceFlow is running inside standalone app mode. Enjoy the full-screen native experience!';
...
function triggerPWAInstall() {
  ...
  deferredPrompt.userChoice.then(function(choiceResult) {
    if (choiceResult.outcome === 'accepted') {
      ...
      showToast('Installing InvoiceFlow...', 'success');
    } else {
